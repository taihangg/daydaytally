...
class SimpleLineChart extends StatefulWidget {
  ...
  SimpleLineChart({
    ...
    boundingTextSize = BoundingTextSize(context);
  ...
  late double _showMaxY;
  late double _showMinY;
  late double _leftSize;
  late double _bottomSize;
  late double _yInterval;
  bool _showZeroY = true;
  bool _showOnLinePoint = false;
  late TextStyle _yStyle = TextStyle(
      color: Colors.black87,
      fontWeight: FontWeight.bold,
      fontSize: _width * 2.5 / 100);

  late TextStyle _xStyle = TextStyle(
      color: Colors.black87,
      fontWeight: FontWeight.bold,
      fontSize: _width * 3 / 100);
...
//        "_showZeroY=$_showZeroY _maxY=${_maxY} _minY=${_minY} _showMaxY=$_showMaxY _showMinY=$_showMinY _yInterval=$_yInterval");
    ...
    // 计算显示y坐标需要预留的左边距宽度
    final intervalStr = _yTitle(_yInterval);
    final maxYStr = _yTitle(_showMaxY);
    final minYStr = _yTitle(_showMinY);
    String yStr = intervalStr;
    if (yStr.length < maxYStr.length) {
      yStr = maxYStr;
    }
    if (yStr.length < minYStr.length) {
      yStr = minYStr;
    }

    // _leftSize = boundingTextSize.getTextSize("0", _yStyle).width;
    // print("_leftSize0=${_leftSize}");

    _leftSize = yStr.length * (_width * 2.5 / 100);
    // print("_leftSize1=${_leftSize} ${_showMaxY}");

    // print("_showMaxY=${_showMaxY} _maxY=${_maxY}");

    _leftSize = boundingTextSize.getTextSize(yStr, _yStyle).width * 1.5;
    // _leftSize =
    //     boundingTextSize.getTextSize(_yTitle(_maxY), _yStyle).width * 1.5;
    // print("_maxY=${_maxY}");

    // print("_leftSize2=${_leftSize}");

    _bottomSize = boundingTextSize.getTextSize("今", _xStyle).height * 1.2;
...
//          fit: BoxFit.fitHeight,
  ...
  List<ShowingTooltipIndicators> _buildShowingTooltipIndicators(
    ...
    for (int lineIndex = 0; lineIndex < lines.length; lineIndex++) {
      ...
      for (int x = 0; x < line.spots.length; x++) {
        list.add(ShowingTooltipIndicators(
            [LineBarSpot(line, lineIndex, line.spots[x])]));
      }
    }
    return list;
  }
...
//    lines.addAll(_buildAveragelines());
    ...
    return LineChartData(
      ...
      gridData: FlGridData(
        ...
        drawVerticalLine: widget._showOnLinePoint ? false : true, // 不显示点的时候显示竖线
      ),
      titlesData: FlTitlesData(
        leftTitles: AxisTitles(
            // TODO:
            // axisNameWidget: Text("abc"),
            // axisNameSize: 15.0,
            // drawBehindEverything: true,
            sideTitles: SideTitles(
          showTitles: true,
          getTitlesWidget: (double value, TitleMeta meta) {
            return Text(widget._yTitle(value), style: widget._yStyle);
          },
        ...
        bottomTitles: AxisTitles(
          sideTitles: SideTitles(
            showTitles: true,
            // interval: 5.0,
            // reservedSize: widget._width * 4 / 100, //下边距
            reservedSize: widget._bottomSize, //下边距
            getTitlesWidget: (double value, TitleMeta meta) {
              final x = value.toInt();
              String str;
              if (x < widget.xTitles.length) {
                str = "${widget.xTitles[x]}";
              } else {
                str = ".$x";
              }
              return Text(str, style: widget._xStyle);
            },
          ...
          getTooltipItems: (List<LineBarSpot> lineBarsSpot) {
            return lineBarsSpot.map((LineBarSpot spot) {
              return LineTooltipItem(
                ...
                TextStyle(
                    fontSize: widget._width * 3 / 100,
                    color: _getColor(widget.lineColors, spot.barIndex + 1)
                        .withOpacity(0.8),
//                    color: (0 == spot.barIndex) ? Colors.red : Colors.grey,
                    fontWeight: FontWeight.bold), // 中间数据的格式
              );
            }).toList();
          }),
    );
  ...
  ExtraLinesData _buildExtraLines() {
    ...
    return ExtraLinesData(
      ...
      horizontalLines: lines.isNotEmpty ? lines : null,
    );
  }
  ...
  FlDotData _buildDotData(int pointCount, double barWidth, int colorIndex) {
    return FlDotData(
      checkToShowDot: (FlSpot spot, LineChartBarData barData) {
        return ((1 == pointCount) || widget._showOnLinePoint) ? true : false;
      },
      getDotPainter: (FlSpot f, double d, LineChartBarData l, int i) {
        return FlDotCirclePainter(
          color: Colors.orangeAccent,
          //_getColor(widget.lineColors, colorIndex),
          radius: 4.0,
          strokeColor: Colors.red,
          // _getColor(widget.lineColors, colorIndex + 3),
...
//      show: false,
        spots: points,
        isCurved: true,
        color: _getColor(widget.lineColors, lineIndex),
        barWidth: barWidth,
