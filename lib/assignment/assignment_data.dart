...
class AssignmentData {
  late int id; // 数据库中的唯一标识
  late String name; // 功课名字
...
primary key (${_columnDate}, ${_columnID})
  ...
  static List<AssignmentData>? _g_assignmentDataList;
  static DateInt? _g_dateInt;
  static late DateTime _g_nextDate;
  static Future<List<AssignmentData>> getAllAssignment() async {
    if (null == _g_assignmentDataList) {
      _g_assignmentDataList = await _queryAllAssignment();
      if (null == _g_dateInt) {
        final today = DateTime.now();
        _g_dateInt = DateInt(today);
        _g_nextDate = DateTime(today.year, today.month, today.day + 1);
        _cycleCheck();
      }
    }
    return _g_assignmentDataList!;
  }

  static int _i = 0;
  static _cycleCheck() async {
    final now = DateTime.now();
    final dateInt = DateInt(now);

    _i++;
    print("cycle check $_i : " + now.toString());

    if (!_g_dateInt!.isSameDay(dateInt)) {
      _g_dateInt = dateInt;
      _g_nextDate = DateTime(now.year, now.month, now.day + 1);

      _g_assignmentDataList = await _queryAllAssignment();
      _event.fire(MyEvent_dataUpdated());
    }

    int sec = _g_nextDate.difference(now).inSeconds;
    if (sec < 10) {
    } else if (sec < 60) {
      sec = 10;
    } else {
...
//        print("update ID=$ID res=$res");
  ...
  Future<int> countPeriodSum(
      DateTime? newBeginDate, DateTime? newEndDate) async {
    int newPeriodSum = 0;
    if ((null == newBeginDate) && (null == newEndDate)) {
      ...
      assert(null != newBeginDate);
      assert(null != newEndDate);
      DateInt newBeginDateInt = DateInt(newBeginDate!);
      DateInt newEndDateInt = DateInt(newEndDate!);
      int beginYear = newBeginDateInt.year;
      int endYear = newEndDateInt.year;
  ...
  Future<String?> apply() async {
    for (final a in _g_assignmentDataList!) {
      if (name == a.name) {
        return "名字已存在";
      }
    }
...
//    await getAllAssignment();
    ...
    _g_assignmentDataList!.add(this);
    _event.fire(MyEvent_dataUpdated());
  ...
  _initSortSequence() {
    sortSequence = _g_assignmentDataList!.length;
    return;
  }
  ...
  void _removeFromList() {
    _g_assignmentDataList!.remove(this);
    return;
  }
  ...
  int todayDone2() {
    ...
    final yearData = _dailyDatas[todayInt.year];
    if (null != yearData) {
      return yearData[todayInt.data] ?? 0;
    }
    return 0;
  }
  ...
  Future<int> todayDone() async {
    DateInt todayInt = DateInt(DateTime.now());
    _lastTodayDone = (await _getDailyDoneAsync(todayInt)) ?? 0;
...
//      return fmt.format(_int2Dt(expirationDate));
  ...
  _setDailyDone(DateInt dateInt, int? oldDone, int newDone) async {
    // assert(null != dateInt);
  ...
  Future<bool> reduceDailyDone(DateTime date, int num,
...
//    DateInt dateInt = startDateInt;
  ...
  todayDoneAddStep() async {
    DateTime today = DateTime.now();
    await addDailyDone(today, step, today);
    return;
  }

  addDailyDone(DateTime date, int num, [DateTime? today]) async {
    // assert(null != date);
    assert(0 <= num);
  ...
  static void import(Map<String, Map<int, int>> datas) async {
    ...
    await importNew(newAssignmentList, datas, batch);

    await importOld(oldAssignmentList, datas, batch);
...
//        final msg = await a.apply();
    ...
    // 提交，以获取ID
    List<dynamic> idList = await batch.commit();
  ...
  Future<String?> applyWithBatch(Batch batch) async {
    ...
    try {
      batch.insert(tableAssignment, _allValuePairs());
//      print("xxx new id=$ID");
    } catch (err) {
      return err.toString();
    }
  ...
  _newAssignmentImportAllYearDailyDatas(
  ...
  _oldAssignmentMergeAllYearDailyDatas(
      Map<int, int> allYearDatas, Batch batch) async {
    for (final e in allYearDatas.entries) {
      final date = e.key;
      final done = e.value;
...
//    await batch.commit();
  ...
  static updateAllAssignmentSortSequence() {
    for (int i = 0; i < _g_assignmentDataList!.length; i++) {
      final a = _g_assignmentDataList![i];
      if (a.sortSequence != i) {
        a.sortSequence = i;
        try {
          _dbMgr.update(
