...
class AssignmentAddEditPage extends StatefulWidget {
  ...
  AssignmentAddEditPage.addNew({required this.onCommitFn}) {
    // assert(null != onCommitFn);
    _title = "添加新功课";
  }

  AssignmentData? oldAssignmentData;
  late final Future<String?> Function() onDeleteFn;
  AssignmentAddEditPage.edit({
    required this.oldAssignmentData,
        ...
        Container(
          ...
          child: FittedBox(
            child: DropdownButton(
              ...
              onChanged: (_DateType? value) async {
                DateTime? dt;
        ...
        Container(
          child: FittedBox(
            child: TextButton(
              style: ButtonStyle(
                  backgroundColor:
                      MaterialStateProperty.all(Color(0xFF00BFFF))),
              child: Text("删除",
                  style: TextStyle(color: Colors.black, fontSize: _width / 20)),
              onPressed: () async {
        ...
        Container(
            child: FittedBox(
                child: TextButton(
                    ...
                    child: Text("返回",
                        style: TextStyle(
                            color: Colors.black, fontSize: _width / 20)),
                    onPressed: () {
                      Navigator.of(context2).pop();
                    }))),
        Container(
          child: FittedBox(
            child: TextButton(
              ...
              onPressed: () async {
                ...
                final String? msg = await widget.onCommitFn(_newAssignmentData);
                if ((null != msg) && ("" != msg)) {
                  ...
                  ScaffoldMessenger.of(context2).showSnackBar(
                    SnackBar(
                      content: Text(
                        msg,
                        style:
                            TextStyle(color: Colors.red, fontSize: _width / 20),
                      ),
                      duration: Duration(seconds: 5),
                      backgroundColor: Colors.tealAccent,
                    ),
