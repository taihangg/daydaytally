...
class DayInfo {
//  #region 日的公历信息
  late int year; // 所在公历年,同lun.y
  late int month; // 所在公历月,同lun.m
  late int day; // 公历日，从1开示
  late int dayIndex; //  所在公历月内日序数
  late double d0; // 2000.0起算儒略日,北京时12:00

//  #region 农历信息
  late int lunarMonth; //农历月，1对应正月
  late int lunarDay; // 农历日，1对应初一
  late String lunarDayName; // 日名称(农历),即'初一,初二等'
  late String lunarRunyue; // 闰状况(值为'闰'或空串)
  late int lunarMonthIndex; //月序号，0对应一月
  late String lunarMonthName; // 月名称
  late int lunarMonthDayCount; //月天数

  //  #region 日的农历纪年、月、日、时及星座
  late double Lyear; // 农历纪年(10进制,1984年起算,分界点可以是立春也可以是春节,在程序中选择一个)
  late double Lmonth; // 纪月处理,1998年12月7日(大雪)开始连续进行节气计数,0为甲子

  late String gzYear; // 干支纪年(立春)
  late String gzMonth; // 干支纪月
  late String gzDay; // 干支纪日
  late String xingzuo; // 星座

  String jieqi = ""; // 节气名称
  late String jieqi2; // 节气名称(实历?)
  late String jqjd; // 节气时刻(儒略日)
  late String jqsj; // 节气时间串

  late double Lyear0; // 农历纪年(10进制,1984年起算)
  late String Lyear3; // 干支纪年(正月)
  late double Lyear4; // 黄帝纪年
  ...
  late double cur_dz; // 距冬至的天数
  late double cur_xz; // 距夏至的天数
  late double cur_lq; // 距立秋的天数
  late double cur_mz; // 距芒种的天数
  late double cur_xs; // 距小暑的天数

  late double Ldn; // 月大小

  late String Lmc2; // 下个月名称,判断除夕时要用到

//  #region 日的其它信息
  late String yxmc; // 月相名称
  late String yxjd; // 月相时刻(儒略日)
  late String yxsj; // 月相时间串

//  #region C#: 从 Javascript 代码中提取出来的其他字段(属性)

  // 每日的十二建信息, 即: {建, 除, 满, 平, 定, 执, 破, 危, 成, 收, 开, 闭} 其中之一
  late String Ri12Jian;
...
// 日历计算类
class LunarMonth {
  late int gregorianYear; // 公历年份
  late int gregorianMonth; // 公历月分
  late int monthDaysCount; // 本月的天数
  late String ganzhiYear; // 该年的干支纪年
  late String shengxiao; // 该年的生肖

  late String nianhao; // 年号
  late int weeksCount; // 本月的总周数

  late int firstWeekday; // 本月第一天的星期
